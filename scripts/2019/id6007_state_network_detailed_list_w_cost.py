...
df = df[~df.state_code.isin(['MS', 'HI'])]
...
state_list = ['AL', 'AR', 'CT', 'DE', 'GA', 'ME', 'NC', 'ND', 'NE', 'RI', 'SC', 'SD', 'UT', 'WA', 'WY', 'WV']
df_upstream_ga_ne = df_sn_spending_district[df_sn_spending_district.state_code.isin(state_list)]

# filtering original df for just states "GA" and "NE"
df_ga_ne = df[['district_id', 'funding_year', 'name', 'state_code',
               'num_students','ia_bw_mbps_total', 'ia_monthly_cost_total']][df.state_code.isin(state_list)]
...
#df_result_ga_ne.to_csv("state_network_detailed_list_w_cost_082919.csv", index=False)
