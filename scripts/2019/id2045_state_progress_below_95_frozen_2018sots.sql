...
states as (select funding_year,
	...
	count(case
		when fiber_districts < .95
			then state_code
		end)
	as state_fiber_districts,
	count(case
		when fiber_campuses < .95
			then state_code
		end)
	as state_fiber_campuses,

	count(case
		when connectivity_districts < .95
			then state_code
		end)
	as state_connectivity_districts,
	count(case
		when connectivity_schools < .95
			then state_code
		end)
	as state_connectivity_schools,
	count(case
		when connectivity_students < .95
			then state_code
		end)
	as state_connectivity_students,
	count(case 
		when connectivity_districts < .95 and fiber_districts< .95
			then state_code
		end)
	as state_fiber_and_connectivity_districts,
	count(case 
		when connectivity_schools < .95 and fiber_campuses < .95
			then state_code
		end)
