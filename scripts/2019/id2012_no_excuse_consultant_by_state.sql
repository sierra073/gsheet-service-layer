...
common_consultants as (
    SELECT
    am.state_code,
    mode() within group (order by consultant_19) as most_common_consultant,
    mode() within group (order by c.consultant_name) as most_common_consultant_in_state

    from all_groups_metrics am

    inner join ps.districts d
    on am.state_code = d.state_code
    and d.in_universe
    ...
    group by am.state_code
)

SELECT
cc.state_code,
cc.most_common_consultant as no_excuse_most_common_in_state,
count(distinct am.district_id) filter(where am.consultant_19 = cc.most_common_consultant)*(select * from get_extrap) as no_excuse_district_count,
count(distinct am.district_id) filter(where am.consultant_19 = cc.most_common_consultant)::numeric/count(distinct am.district_id)::numeric as "% of No Excuse on most common consultant",
CASE
  WHEN most_common_consultant = most_common_consultant_in_state
    then false
  ELSE true
END as diff_from_state_most_common

from all_groups_metrics am

inner join common_consultants cc
on am.state_code = cc.state_code

where funnel_group = 'No Upgrade No Excuse'

group by cc.state_code,
most_common_consultant,
most_common_consultant_in_state

order by "% of No Excuse on most common consultant" desc
