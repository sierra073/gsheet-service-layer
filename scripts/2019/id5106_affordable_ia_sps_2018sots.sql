...
count(distinct case when sp.district_id = f.district_id then primary_sp end)::numeric as sp_num

from ps.districts_sp_assignments sp
left join 
(select * from final where sp_offering_deal_nationally = true) f
on sp.district_id = f.district_id

where sp.funding_year = 2019
and sp.primary_sp is not null
