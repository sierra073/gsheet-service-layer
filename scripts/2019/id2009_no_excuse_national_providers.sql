...
sp_percents as ( -- get national counts for each provider
    with district_sp as (
        SELECT
        d.district_id,
        ds.primary_sp
        ...
        left join ps.districts_sp_assignments ds
        on d.district_id = ds.district_id
        and d.funding_year = ds.funding_year
    ...
    count(distinct district_id)::numeric/(select count(*)::numeric from district_sp) as natl_perc
...
row_number() over(order by spp.district_count desc) as national_rank

from all_groups_metrics am

join sp_percents spp
on am.sp_19 = spp.primary_sp

where funnel_group = 'No Upgrade No Excuse'

group by sp_19,spp.district_count
