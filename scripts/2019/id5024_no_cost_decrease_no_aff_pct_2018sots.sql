...
select 
  ...
  dbc.meeting_knapsack_affordability_target,
...
select '1) # districts that havent seen a cost decrease and still not meeting benchmark prices (extrap)' as metric,

(sum(case when cost_decrease_indicator=false and meeting_knapsack_affordability_target = false then ndistricts end)::numeric /
sum(ndistricts)) * 
(select count(*)
...
select '2) % districts that havent seen a cost decrease that still arent meeting benchmark prices' as metric,
(sum(case when  meeting_knapsack_affordability_target = false then ndistricts end)::numeric /
...
select '3) % districts that havent seen a cost decrease that still arent meeting benchmark prices overall' as metric,
