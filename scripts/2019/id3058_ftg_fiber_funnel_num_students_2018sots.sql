...
SELECT SUM(CASE WHEN d.state_code = 'AK'
          ...
          end) AS ak_num_unscalable,
      SUM(CASE WHEN d.state_code != 'AK'
          THEN df.known_unscalable_campuses + df.assumed_unscalable_campuses
          end) AS nat_num_unscalable
        FROM ps.districts d
...
unscalable_campus_sample AS ( --sums all unscalable campuses and removes AK for extrapolations
  ...
  SUM(CASE
        ...
        OR  cfa.campus_fit_for_campus = false
  ...
  * ak_num_unscalable
    /SUM(CASE
        ...
        THEN 1
      END)
   AS ak_sum_unscalable_campuses,--(147)
   SUM(CASE
         WHEN d.state_code != 'AK'
         and df.fiber_target_status = 'Target'
         AND ( cfa.campus_fit_for_campus = true
...
ON cfa.campus_id = lycfa.campus_id
AND cfa.funding_year = lycfa.funding_year +1

  LEFT JOIN ps.fiber_distance_static fds
...
--Free State Iterations IL,MO,WA have free state matches
  ...
  SUM(CASE
        WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                          'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
        AND d.state_code NOT IN ('IL','MO','WA')
