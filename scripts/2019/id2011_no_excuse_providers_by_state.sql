...
common_providers as (
    SELECT
    am.state_code,
    mode() within group (order by sp_19) as most_common_sp,
    mode() within group (order by primary_sp) filter(where fit.fit_for_ia = true) as most_common_sp_in_state
...
SELECT
cp.state_code,
cp.most_common_sp as no_excuse_most_common_in_state,
count(distinct am.district_id) filter(where am.sp_19 = cp.most_common_sp)*(select * from get_extrap) as no_excuse_district_count,
count(distinct am.district_id) filter(where am.sp_19 = cp.most_common_sp)::numeric/count(distinct am.district_id)::numeric as "% of No Excuse on most common SP",
CASE
  WHEN most_common_sp = most_common_sp_in_state
    then false
  ELSE true
...
inner join common_providers cp
on am.state_code = cp.state_code

where funnel_group = 'No Upgrade No Excuse'

group by cp.state_code,most_common_sp,most_common_sp_in_state

order by "% of No Excuse on most common SP" desc
