...
(with not_meeting_w_cost as (
   ...
   --END OF MEGA CONCURRENCY
      ...
      meet_at_1gbps as (
          ...
          END as meets_w_1g,
          CASE
            WHEN (10000000/num_students)::numeric/ (CASE -- same as in ps.districts_bw_cost
                                                      WHEN size = 'Medium'
                                                        then .85
                                                      WHEN size = 'Large'
                                                        then .7
      ...
      SELECT
      '1. All not in state network' as id,
      ...
      and state_network_tf = false

      UNION

      SELECT
      '2. Meeting 1Mbps' as id,
      count(distinct nm.district_id) as sample_districts,
      count(distinct nm.district_id)*(select dist_multiplier from get_extrap) as total_districts,
      sum(num_students) as sample_students,
      sum(num_students)*(select stud_multiplier from get_extrap) as total_students
      ...
      SELECT
      '6. Would meet w 10gps only' as id,
      count(distinct nm.district_id) as sample_districts,
      count(distinct nm.district_id)*(select dist_multiplier from get_extrap) as total_districts,
      sum(num_students) as sample_students,
      sum(num_students)*(select stud_multiplier from get_extrap) as total_students
      ...
      and not meets_w_1g
      and meets_w_10g
      and nm.ia_monthly_cost_total >= 2500) t
