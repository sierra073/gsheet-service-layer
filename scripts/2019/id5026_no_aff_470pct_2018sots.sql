with t as (select 
  ...
  case when dbc.ia_bw_mbps_total > dbc2.ia_bw_mbps_total
    then true
  else false end as increased_bw,
...
select count(case when filed_470 = true and no_decrease_no_aff = true then district_id end)/
count(case when no_decrease_no_aff = true then district_id end)::numeric as pct_filed_470
from t