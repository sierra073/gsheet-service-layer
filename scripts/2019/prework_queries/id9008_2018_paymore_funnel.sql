...
pay_more_movers as (
    ...
    case when du.path_to_meet_2018_goal_group = 'New Knapsack Pricing' then 'Pay More'
      else du.path_to_meet_2018_goal_group end as "FY19 Group"
    ...
    and (du.path_to_meet_2018_goal_group is not null or dbc.meeting_2018_goal_oversub = true)
    and fit_for_ia_cost)

select
  case when meeting_2018_goal_oversub = true then 'Newly Meeting'
  else "FY19 Group" end as status_2019,
  count(distinct district_id) as nd
from pay_more_movers
group by 1
union
select
  'Total' as status_2019,
  count(distinct district_id) as nd
  from pay_more_movers
group by 1
