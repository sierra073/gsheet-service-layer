...
select 
'districts' as subgroup,
case when round(c1_discount_rate*100,0) <= 50 then round(c1_discount_rate*100,0)
when round(c1_discount_rate*100,0) between 60 and 70 then 70
else round(c1_discount_rate*100,0) end as c1_discount_rate, 
count(*)::numeric/(select count(*) from final) as pct_districts
from final
where c1_discount_rate is not null
group by 1,2
...
count(distinct district_id)::numeric/(select count(*) from dpr) as pct_districts
from dpr
where c1_discount_rate is not null
group by 1,2
