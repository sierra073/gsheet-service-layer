...
df_jamie = pd.concat([df_dar_no_cost_pd, df_jamie_csv])
...
df_combo_dar_js = pd.concat(frames)
...
# sn_states = ['AL', 'AR', 'DE', 'GA', 'KY', 'NC', 'ND', 'NE', 'RI', 'SC', 'SD', 'UT','WA', 'WV', 'WY']
# other_states = ['CT', 'ME', 'DC', 'HI', 'MS']

# add num_districts_meeting_2019
# df_2019 = df_d_dffa[(df_d_dffa.fit_for_ia == True) &
#          (df_d_dffa.fit_for_ia_cost == True) &
#          (df_d_dffa.in_universe == True) &
#          (df_d_dffa['district_type']=='Traditional') &
#          (df_d_dffa.meeting_2018_goal_no_oversub == True) &
#           (df_d_dffa.state_code.isin(df_2023.state_code.unique()))
#          ].groupby('state_code').agg({'district_id': 'count',
#                                       'num_students': 'sum'}).fillna(0).astype(int).reset_index()

### rerun version 1: removing fit_for_ia_cost
...
# remove state network states
# df_2019 = df_2019[~df_2019.state_code.isin(sn_states+other_states)]

### rerun version 2: ALL states (non state networks and state networks)
...
# concat with
df_districts_pct_js = pd.concat([df_2023, df_2019])
...
# save as csv
df_students_pct_all.to_csv("/Users/katherineaquino/Desktop/df_students_pct_all_092419.csv", index=False)
