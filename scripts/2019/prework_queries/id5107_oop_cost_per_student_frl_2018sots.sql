...
num_students,
ia_annual_cost_oop,
ia_annual_cost_oop/num_students as ia_annual_cost_oop_per_student,
frl_percent
...
where no_cost_peer_deal = FALSE 
AND consortia_already_meeting = FALSE
and meeting_2018_goal_oversub = FALSE
and fit_for_ia_cost = TRUE)

select 
    'meeting 1 Mbps' as subgroup,
    frl_percent*100::numeric as frl_percent,
    (ia_annual_cost_total - ia_funding_requested_erate)/num_students as ia_annual_oop_cost_per_student
  from subset
  
  where fit_for_ia_cost = true
  and meeting_2018_goal_oversub = true
  and frl_percent is not null

union

select 'need to spend more to meet' as subgroup,
frl_percent*100::numeric as frl_percent,
ia_annual_cost_oop_per_student
from grp
where frl_percent is not null
order by 1,2