...
not_meeting as (
  select 
    round(sample_groups_pre.num_students*not_meeting_pre.num_students::numeric/samples.num_students,0) as num_students,
    round(sample_groups_pre.peer_oop_increase*not_meeting_pre.num_students::numeric/samples.num_students,0) as peer_oop_increase
    from sample_groups_pre
    join not_meeting_pre
    on true
    join (
    select
      sum(num_students) as num_students
    ...
    on true
    where sample_groups_pre.subgroup = 'increase budget' 
),
...
select
  sample_groups.state_code,
  round(sample_groups.peer_oop_increase*not_meeting.peer_oop_increase::numeric/samples.peer_oop_increase,0) as peer_oop_increase
  from sample_groups
  join not_meeting
  on true
  join (
    select
      sum(peer_oop_increase) as peer_oop_increase,
      sum(num_students) as num_students
    from sample_groups
  ) samples
  on true
