with peer_deal_sample as (select dd.funding_year,
	dd.district_id,
dd.num_students,
case 
...
round((sum(case 
	when d.line_item_deal = True
		then d.num_students end)::numeric/sum(d.num_students))*extrapolated_students_not_meeting) 
as students_w_deal_extrap,
extrapolated_students_not_meeting
from peer_deal_sample d 

join extrap_pop
on extrap_pop.funding_year = d.funding_year
