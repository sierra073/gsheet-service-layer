...
select median(case when (cost_diff_per_student > 0)
    then cost_diff_per_student
    end) as median_added,
    AVG(case when (cost_diff_per_student > 0)
    then cost_diff_per_student
    end) as mean_added,
    median(case when (cost_diff_per_student <= 0)
    then cost_diff_per_student
    end) as median_saved,
    avg(case when (cost_diff_per_student <= 0)
    then cost_diff_per_student
