...
fiber_on_block_sample as ( /*this is the sample group from distances_fiber_static that we trust the distances fiber locator provided
  ...
  /SUM(CASE
        WHEN (sp.primary_sp NOT IN ('AT&T','CenturyLink','Comcast','Spectrum')
        AND sp.primary_sp NOT ILIKE '%Verizon%')
  THEN 1 ELSE 0 END)::numeric  as no_inc_pct_fiber_on_block, --(.168)
  SUM(CASE
...
--State Match Percent Iterations
  SUM(CASE
        WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
          ...
          ELSE 0
      END)::numeric
  /COUNT(*)::numeric AS state_match_all, --(.711)
  SUM(CASE
        WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                          'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
        AND (fds.distance_to_fiber > 750
...
--Free State Iterations IL,MO,WA have free state matches
  ...
  SUM(CASE
        WHEN d.state_code IN ('AZ','IL','MO','WA')
          THEN 1
          ELSE 0 END)
  /COUNT(*)::numeric AS state_full_match_all, --(.123)
  SUM(CASE
        WHEN d.state_code IN ('AZ','IL','MO','WA')
        AND (fds.distance_to_fiber > 750
        OR fds.distance_to_fiber IS NULL)
  ...
  SUM(CASE
        ...
        AND ftg.median_total_district_cost = 0
          ...
          ELSE 0
      END)::numeric
  /COUNT(*)::numeric AS free_state_match_all, --(.376)
  SUM(CASE
        WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                          'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
        AND d.state_code NOT IN ('AZ','IL','MO','WA')
  ...
  /SUM(CASE
    ...
    SUM(CASE
          ...
          AND ftg.median_total_district_cost <= 30000
            ...
            ELSE 0
        END)::numeric
    /COUNT(*)::numeric AS low_cost_state_match_all, --(.043)
    SUM(CASE
          WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                            'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
          AND d.state_code NOT IN ('AZ','IL','MO','WA')
    ...
    SUM(CASE
          ...
          AND ftg.median_total_district_cost > 30000
            ...
            ELSE 0
        END)::numeric
    /COUNT(*)::numeric AS high_cost_state_match_all, --(.169)
    SUM(CASE
          WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                            'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
          AND d.state_code NOT IN ('AZ','IL','MO','WA')
          ...
          AND (ftg.median_total_cost
                * (1- CASE
                    WHEN d.c1_discount_rate IS NULL
                      THEN .7
                      ELSE d.c1_discount_rate END)) <= 30000
            THEN 1
            ELSE 0
        END)::numeric
    /COUNT(*)::numeric AS low_cost_no_match_all, --(.004)
    SUM(CASE
          WHEN d.state_code NOT in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                            'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
          AND (fds.distance_to_fiber > 750
    ...
    SUM(CASE
          ...
          AND (ftg.median_total_cost
            ...
            ELSE 0
        END)::numeric
    /COUNT(*)::numeric AS high_cost_no_match_all, --(.285)
    SUM(CASE
          WHEN d.state_code NOT in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                            'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
          AND (fds.distance_to_fiber > 750
  ...
  SUM(CASE
        ...
        AND ftg.median_total_district_cost <= 30000
          ...
          ELSE 0
      END)::numeric
  /COUNT(*)::numeric AS low_cost_if_had_match_all, --(.142)
  SUM(CASE
        WHEN d.state_code NOT in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                          'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
        AND (fds.distance_to_fiber > 750
  ...
  SUM(CASE
        ...
        AND ftg.median_total_district_cost > 30000
          ...
          ELSE 0
      END)::numeric
  /COUNT(*)::numeric AS high_cost_if_had_match_all, --(.148)
  SUM(CASE
        WHEN d.state_code NOT in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                          'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
        AND (fds.distance_to_fiber > 750
...
SELECT
d.state_code,
SUM(d.num_campuses) as campuses_in_state,
SUM(df.assumed_unscalable_campuses + df.known_unscalable_campuses)
        AS unscalable_campuses_by_state,
 CASE
    WHEN d.state_code IN ('AZ','IL','MO','WA')
      THEN SUM( df.assumed_unscalable_campuses + df.known_unscalable_campuses)
    ELSE SUM(df.assumed_unscalable_campuses + df.known_unscalable_campuses) * fob.hybrid_pct_fiber_on_block --fob
        + SUM(df.assumed_unscalable_campuses + df.known_unscalable_campuses) * (1 - fob.hybrid_pct_fiber_on_block)
          * bcc.free_state_match_needs_build -- free state match
        + SUM(df.assumed_unscalable_campuses + df.known_unscalable_campuses) * (1 - fob.hybrid_pct_fiber_on_block)
          * bcc.low_cost_state_match_needs_build --low_cost_state_match
        + SUM(df.assumed_unscalable_campuses + df.known_unscalable_campuses) * (1 - fob.hybrid_pct_fiber_on_block)
          * bcc.low_cost_no_match_needs_build --low_cost_no_match
  END AS campuses_upgraded,
  CASE
    WHEN d.state_code IN ('AZ','IL','MO','WA')
      THEN 0
    ELSE SUM(df.assumed_unscalable_campuses + df.known_unscalable_campuses) * (1 - fob.hybrid_pct_fiber_on_block)
        * bcc.high_cost_state_match_needs_build -- match high cost
        + SUM(df.assumed_unscalable_campuses + df.known_unscalable_campuses) * (1 - fob.hybrid_pct_fiber_on_block)
            * bcc.high_cost_no_match_needs_build -- no match high cost
    END AS campuses_remaining
FROM ps.districts d
...
LEFT JOIN build_cost_categories bcc
ON true

LEFT JOIN fiber_on_block_sample fob
ON true
...
GROUP BY d.state_code,
        ...
        --campuses_upgraded
        fob.hybrid_pct_fiber_on_block,
        bcc.free_state_match_needs_build,
        bcc.low_cost_state_match_needs_build,
        bcc.low_cost_no_match_needs_build,

        --campuses remaining
        bcc.high_cost_state_match_needs_build,
        bcc.high_cost_no_match_needs_build

      )

SELECT COUNT(CASE WHEN spc.campuses_remaining/spc.campuses_in_state < .015
                   AND spc.unscalable_campuses_by_state/spc.campuses_in_state >= .015
                THEN 1 ELSE NULL end)
FROM state_pct_fiber spc
