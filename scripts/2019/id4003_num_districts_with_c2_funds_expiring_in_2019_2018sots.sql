...
--needed to start their clock in 2014 so they should be in their 4th year of their funds
and y.num_years_funds_used = 4)

select count(district_id) as num_districts_with_c2_funds_expiring_in_2019,
       count(district_id) filter (where perc_remaining_postdiscount > .5) as num_districts_with_more_than_50percent_c2_funds_left_expiring_in_2019
from agg
