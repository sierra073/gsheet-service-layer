...
SUM (CASE WHEN c.campus_id IN
      ...
      THEN c.num_students
ELSE 0 END)/SUM(c.num_students)::numeric as percent_fiber_at_door,
SUM (CASE WHEN c.campus_id IN
        ...
        THEN c.num_students
ELSE 0 END)/SUM(c.num_students)::numeric as percent_fiber_on_block
...
percents as (
  SELECT SUM(CASE WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
            ...
            THEN ((df.assumed_unscalable_campuses + df.known_unscalable_campuses)/d.num_campuses  * d.num_students)
            ELSE 0
      END)/SUM((df.assumed_unscalable_campuses + df.known_unscalable_campuses)/d.num_campuses  * d.num_students)
       AS state_match_percent_w_exp,
      ...
      SUM(CASE WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','MA','MD','ME',
            ...
            ELSE 0
      END)/SUM((df.assumed_unscalable_campuses + df.known_unscalable_campuses)/d.num_campuses  * d.num_students)
       AS free_state_match_percent_no_exp
  FROM ps.districts d
...
num_unscalable AS (
SELECT SUM((df.assumed_unscalable_campuses + df.known_unscalable_campuses)/d.num_campuses  * d.num_students) as unscalable_students
...
AND fit.fit_for_wan = true
 ...
 SELECT
      ROUND((nu.unscalable_students* (1-fad.percent_fiber_on_block) --need build
      * (1- state_match_percent_w_exp))) AS no_state_match_non_fiber
