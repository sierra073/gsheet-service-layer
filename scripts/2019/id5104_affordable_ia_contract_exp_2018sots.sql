with subset as (
  ...
  --for prior year contract end date
  JOIN ps.districts_lines dlpy
  ON dfa.district_id = dlpy.district_id
  AND dfa.funding_year -1 = dlpy.funding_year
...
sample_groups1 as (
  ...
  most_recent_ia_contract_end_date_py,
  upgrade_indicator,
  num_broadband_470s,
...
select 
count(final.district_id)::numeric/
(select count(*) as total_districts from final) as pct_districts
from final
where most_recent_ia_contract_end_date_py::date <= '2019-08-31'
