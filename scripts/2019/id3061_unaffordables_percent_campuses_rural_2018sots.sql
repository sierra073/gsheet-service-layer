...
--Free State Iterations IL,MO,WA have free state matches
  ...
  /SUM(CASE
        ...
        --This calculates then pct of students in high cost no match campuses that are rural/small town
        SUM(CASE
              WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                                'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
              AND d.state_code NOT IN ('AZ','IL','MO','WA')
              ...
              OR fds.distance_to_fiber IS NULL)
              AND ftg.median_total_district_cost > 30000
              AND d.locale IN ('Rural','Small Town')
                THEN 1
                ELSE 0
            END)::numeric
        /SUM(CASE
              ...
              OR fds.distance_to_fiber IS NULL)
                THEN 1
                ELSE 0 END)::numeric AS high_cost_rural_state_match_needs_build,
          SUM(CASE
                WHEN d.state_code NOT in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                                  'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
                AND (fds.distance_to_fiber > 750
          ...
          /SUM(CASE
               ...
               AND d.locale IS NOT NULL
                  THEN 1
                  ELSE 0 END)::numeric AS high_cost_rural_no_match_needs_build, --(.266)
          --
          SUM(CASE
                WHEN d.state_code in ('AZ','CA','CO','FL','ID','IL','KS','MA','MD','ME',
                                  'MO','MT','NC','NH','NM','NV','NY','OK', 'OR','TX','VA','WA','WI')
          ...
          /SUM(CASE
               ...
               AND d.locale IS NOT NULL
                  THEN 1
                  ELSE 0 END)::numeric  AS high_cost_rural_needs_build --both state match and no match
...
SELECT --ucs.state_code,
      ...
      * bcc.high_cost_rural_state_match_needs_build)
      /(ucs.nat_sum_unscalable_campuses_no_ak * (1 - fob.hybrid_pct_fiber_on_block)
      * bcc.high_cost_state_match_needs_build) AS high_cost_state_match,
      (ucs.nat_sum_unscalable_campuses_no_ak * (1 - fob.hybrid_pct_fiber_on_block)
      * bcc.high_cost_rural_no_match_needs_build)
      /(ucs.nat_sum_unscalable_campuses_no_ak * (1 - fob.hybrid_pct_fiber_on_block)
      * bcc.high_cost_no_match_needs_build) AS high_cost_no_match,

      (ucs.nat_sum_unscalable_campuses_no_ak * (1 - fob.hybrid_pct_fiber_on_block)
      * bcc.high_cost_rural_needs_build)
      /((ucs.nat_sum_unscalable_campuses_no_ak * (1 - fob.hybrid_pct_fiber_on_block)
      * bcc.high_cost_state_match_needs_build)
      +(ucs.nat_sum_unscalable_campuses_no_ak * (1 - fob.hybrid_pct_fiber_on_block)
      * bcc.high_cost_no_match_needs_build)) AS high_cost_unaffordable
