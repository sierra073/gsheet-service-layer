...
x = pd.crosstab(df.no_decrease_no_aff,df.upgrade_indicator).apply(lambda r: r/r.sum(), axis=1).reset_index()
...
for i in ax.patches:
    ...
    ax.text(i.get_x()+.17, i.get_height()+.01, \
            str(round(vec[j]*100,1))+'%', fontsize=12)
    j+=1

# save image
os.chdir(GITHUB + '/''figure_images')
plt.savefig('id5025_no_aff_upgrade_2018sots.png')
