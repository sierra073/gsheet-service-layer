...
not_meeting as (
  select 
    ...
    case
      when largest = true
        then sample_students
      else (sum(population_students) OVER ()) - (max(sample_students) OVER ())
    end as population_students,
    round(case
            when largest = true
              then sample_students_not_meeting
            else ((sum(sample_students_not_meeting) OVER ())/(sum(sample_students) OVER ())*
                    (sum(population_students) OVER ())) - 
                      (max(sample_students_not_meeting) OVER ())
          end ,0) as population_students_not_meeting
    from (
      select 
        num_students > 9000 as largest,
...
from sample
join not_meeting
on sample.largest = not_meeting.largest
group by 1, not_meeting.population_students_not_meeting