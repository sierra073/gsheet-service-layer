...
unscalable_campus_sample AS ( --sums all unscalable campuses and removes AK for extrapolations
  ...
  SELECT (SUM(CASE WHEN d.state_code = 'AK'
                  AND df.known_unscalable_campuses + df.assumed_unscalable_campuses > 0
                  THEN d.num_students END)
        * SUM(CASE WHEN d.state_code = 'AK'
            THEN df.known_unscalable_campuses + df.assumed_unscalable_campuses
        end)) /
        SUM(CASE WHEN d.state_code = 'AK'
            AND df.known_unscalable_campuses + df.assumed_unscalable_campuses > 0
            THEN d.num_campuses
        end) AS ak_sum_unscalable_campuses,
        (SUM(CASE WHEN d.state_code != 'AK'
                        AND df.known_unscalable_campuses + df.assumed_unscalable_campuses > 0
                        THEN d.num_students END)
              * SUM(CASE WHEN d.state_code != 'AK'
              ...
              SUM(CASE WHEN d.state_code != 'AK'
                  AND df.known_unscalable_campuses + df.assumed_unscalable_campuses > 0
                  THEN d.num_campuses
              end) AS nat_sum_unscalable_campuses_no_ak
          FROM ps.districts d
...
--***Below are percents requested for follow up metrics That are not used in the funnel***
