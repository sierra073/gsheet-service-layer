...
# reverse index order 
df_pct_change.sort_index(axis='index',ascending=False,inplace=True)
...
plt.bar(xmarks, df_pct_change['district_running_total'], color='#009296')

# ticks, titles, axis labels
plt.ylabel("Percent of Total Districts with Best Deal")
plt.xlabel("Bandwidth Increase with Best Deal")
plt.xticks(xmarks, df_pct_change.pct_category.values, rotation=20)
plt.box(on=None)
plt.yticks([])
for x0,v0, label in zip (xmarks,df_pct_change['district_running_total'],
                         df_pct_change['district_running_total_percent_total']):
    plt.text(x0-.25,v0+10,str(int(round(label*100)))+'%')
