...
What percentage of districts continue to upgrade after they meet 1 Mbps?
'''
...
# list of districts_ids meeting 1Mbps at anytime between 2015-2019
all_districts_meeting1m = df_final_ia[df_final_ia.meeting_2018_goal_oversub == True].district_id.unique()

for year in [2015, 2017, 2018, 2019]:
    if year == 2015:
        # list of district_ids meeting 1M in 2015
        districts_meeting1m_year = df_final_ia[(df_final_ia.funding_year == year) &
                                              (df_final_ia.meeting_2018_goal_oversub == True)].district_id.unique()
    else:
        reduced_districts_list = set(all_districts_meeting1m).difference(set(subset_district_ids))
        districts_meeting1m_year = df_final_ia[(df_final_ia.funding_year == year) &
    ...
    subset_district_ids.extend(districts_meeting1m_year)

# results as df
df_results = pd.DataFrame.from_dict(results, orient='index').reset_index()

# renaming columns
df_results.columns = ['district_id', 'year_first_meet1m', 'year_first_upgrade', 'year_latest_upgrade']
...
df_results.loc[:, 'years_to_upgrade_after_meeting1m'] = df_results['year_first_upgrade'] - df_results['year_first_meet1m']

# sort by district_id
df_results.sort_values('district_id', inplace=True)

# Revised Metric: XX% Districts continued to upgrade after meeting 1Mbps
# Of districts that met 100kbps goal between 2015-2018, what percentage continued to upgrade after meeting 1Mbps?
num_meeting1m_15to17 = df_results[(df_results.year_first_meet1m < 2019)].district_id.nunique()
num_upgraded = df_results[(df_results.year_first_meet1m < 2019) &
                          (df_results.years_to_upgrade_after_meeting1m != 0)].district_id.nunique()
print(num_upgraded/num_meeting1m_15to17)
